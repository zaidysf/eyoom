...
if (file_exists($dbconfig_file)) {
    ...
    include_once(G5_PATH.'/install/eyoom_index.php');
?>
...
if (isset($_REQUEST['sca']))  {
    $sca = filter_var($_REQUEST['sca'], FILTER_VALIDATE_REGEXP, array(
        "options" => array("regexp" => "/^[^<>'\"%=\(\)\/\^\*]+$/")
    ));
    if ($sca) {
        $sca = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", clean_xss_tags(trim($sca)));
...
if (isset($_REQUEST['sfl']))  {
    $sfl = filter_var($_REQUEST['sfl'], FILTER_VALIDATE_REGEXP, array(
        "options" => array("regexp" => "/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*\s\#\._]/")
    ));
    $sfl = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*\s\#]/", "", trim($sfl));
    if ($sfl)
...
if (isset($_REQUEST['bo_table']) && ! is_array($_REQUEST['bo_table'])) {
    $bo_table = filter_var($_REQUEST['bo_table'], FILTER_VALIDATE_REGEXP, array(
...
if (isset($_REQUEST['gr_id'])) {
    if (!is_array($_REQUEST['gr_id'])) {
        $gr_id = filter_var($_REQUEST['gr_id'], FILTER_VALIDATE_REGEXP, array(
            "options" => array("regexp" => "/^[a-z0-9_]+$/i")
        ));
        $gr_id = preg_replace('/[^a-z0-9_]/i', '', trim($gr_id));
    }
} else {
    $gr_id = '';
}
...
if (isset($_REQUEST['fm_id'])) {
    if (!is_array($_REQUEST['fm_id'])) {
        $fm_id = filter_var($_REQUEST['fm_id'], FILTER_VALIDATE_INT);
    }
} else {
    $fm_id = 1;
}
...
if (isset($_REQUEST['pid'])) {
    if (!is_array($_REQUEST['pid'])) {
        $pid = filter_var($_REQUEST['pid'], FILTER_VALIDATE_REGEXP, array(
            "options" => array("regexp" => "/^[a-z0-9_|]+$/i")
        ));
        $pid = preg_replace('/[^a-z0-9_|]/i', '', trim($pid));
        $pid = substr($pid, 0, 20);
    }
} else {
    $pid = '';
